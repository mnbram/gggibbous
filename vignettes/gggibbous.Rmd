...
```{r, echo = FALSE, fig.width = 3, fig.height = 1.5, out.extra = 'style="border: none; display: block; margin-left: auto; margin-right: auto"'}
ggplot(
  data.frame(
    y = c(1, 3), group = c("a", "b"),
  ...
  geom_col(aes(x = factor(0)), width = 2, color = "black") +
  geom_moon(
    data = data.frame(
      x = factor(rep(-0.5, 2)), y = rep(0, 2), ratio = c(0.75, 0.25),
      right = c(TRUE, FALSE), group = c("b", "a"), facet = c("moon", "moon")
    ),
    aes(x = x, y = y, ratio = ratio, right = right), size = 36, stroke = 0.5
  ) +
  coord_polar(theta = "y") +
  facet_wrap(~facet) +
...
```{r, echo = FALSE, fig.width = 5, fig.height = 1.25, out.extra = 'style="border: none; display: block; margin-left: auto; margin-right: auto"'}
